...
type Guru = { id: string; nama: string; kodeGuru: string };
...
export default function GeneratePiketForm({ guruList }: { guruList: Guru[] }) {
  ...
  const allChecked = dipilih.size === guruList.length;
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-blue-100 bg-blue-50 px-4 py-3 text-sm text-blue-800">
        ...
        <ul className="mt-1 space-y-0.5 text-xs text-blue-700 list-disc pl-4">
          <li>
            <span className="font-semibold text-amber-700">1 hari Piket Harian</span> — distribusi merata Senin–Sabtu
          </li>
          <li>
            <span className="font-semibold text-purple-700">1 hari Piket Karakter</span> — di hari <em>berbeda</em> dari Harian
          </li>
        </ul>
      </div>
          ...
          <div className="divide-y divide-zinc-100">
            {guruList.map((g) => (
              ...
              >
                ...
                <span className="text-sm text-zinc-800">{g.nama}</span>
              </label>
            ))}
          </div>
        </div>
